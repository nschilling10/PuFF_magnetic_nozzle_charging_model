...
for i=1:length(NT1_vec)
    for j=1:length(NT2_vec)
        ...
        tic
        [I1_max,capIsCharged]=rlc_recharge_nozzle_circuit_v3_6(circInps,plasmaInps,graphDisplay);
        toc
        
        if capIsCharged
            Min_current_ratio_mat(i,j)=I1_max/circInps.I10;
        end
    end
end
%% Visualization
...
%% Find min
[rows,colInd_vec]=min(EtaSys_mat);
[val,rowInd]=min(rows);
