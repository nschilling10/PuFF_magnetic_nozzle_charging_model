...
%         end
    ...
    voltage_in_circuit=L_nozz_hand(t).*[(diff(I_1)./diff(t));0]+dL_nozz_hand(t).*I_1;
    
    voltage_L1=input.L1.*[(diff(I_1)./diff(t));0];
    voltage_L2=input.L2.*[(diff(I_2)./diff(t));0];
    ...
    if circuitInputParams.graphDisplay
        ...
        V_loop1= voltage_in_circuit+I_1*input.R1+voltage_L1 - M_circ*[(diff(I_2)./diff(t));0]+input.l_0*[(diff(I_1)./diff(t));0];
        V_loop2= voltage_L2+I_2*input.R2+V_Cap - M_circ*([(diff(I_1)./diff(t));0]);
        
        figure(7)
        plot(t*1e6,V_loop1,t*1e6,V_loop2)
        %plot(t(1:end-1)*1e6,V_loop2(1:end-1))
        xlabel('Time since ignition (\mus)')
...
%         L_0*dI(1,1)+(R_1+holderArray.dLt_handle(t))*I1-M*dI(3,1)
%         L_2*dI(3,1)+I2*R_2+V_cap - M*dI(1,1)
    end
end