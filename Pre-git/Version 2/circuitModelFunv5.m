% Nathan Schilling
% 02/19/19
function [E_gain,E_circ] = circuitModelFunv5(circuitInputParams)
...
%         set(gca,'LineWidth',2)
    ...
    figure(5)
    plot(t*10^6,E_gen*10^-6,t*10^6,E_R1*10^-6)
    xlabel('Time since ignition (\mus)')
    ylabel('Energy in component (MJ)')
    legend('Generator','R_1','Location','Northwest')
    figure(6)
    ...
    function dI = fcgfuns(t,y,holderArray)
        ...
        dI=zeros(2,1);
        dI(1,1)=((R_1-holderArray.dLt_handle(t))*I1+V_cap)/L_0;
        dI(2,1)=I1/C_load;
    end
end