...
%     end
    ...
    function out=L_nozz_hand(x)
        s=0.5;
        if x < tau
            out=L0*((-0.9/tau^2)*(1/(s^2-s))*x.^2+(0.9/tau)*(1/(s^2-s))*x+1);
        else
            out=0;
        end
    end
    ...
    function out=dL_nozz_hand(x)
        s=0.5;
        if x < tau/2
            out=L0*(2*(-0.9/tau^2)*(1/(s^2-s))*x+(0.9/tau)*(1/(s^2-s)));
        else
            out=0;
        end
    end
    ...
    voltage_L2=input.L2.*[0;(diff(I_1)./diff(t))];
...
%         plot(t,E_tot)
    ...
    figure(8)
    plot(t*1e6,voltage_L1.*I_2,t*1e6,voltage_L2.*I_2)
    xlabel('Time since ignition (\mus)')
    ylabel('Power')
    figure(3)
    end
