...
%         end
    ...
    dI_mat=zeros(length(tSpan),3);
    
    for j=1:length(tSpan)
        dI_mat(j,:)=fcgfuns(t(j),y(j,:),input);
    end
    ...
    if circuitInputParams.graphDisplay
        ...
        % Differential Voltage Definition
...
%        E_gen_ind=0.5*L_nozz_hand(t).*I_1.^2;
        ...
        %         plot(t,E_tot)
        figure(5)
        plot(t*10^6,E_gen*10^-6,t*10^6,E_L1*10^-6,t*10^6,E_L2*10^-6,t*10^6,E_M*10^-6,t*10^6,E_R1*10^-6,t*10^6,E_R2*10^-6)
        xlabel('Time since ignition (\mus)')
