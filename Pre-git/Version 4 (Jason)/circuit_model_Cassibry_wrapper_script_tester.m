...
input.v_exp_hand= @(t) sqrt(g*R_gas*2*T)*(cos(pi*t/(2*input.tau)).*...
    ((t/input.tau)<2)-(1.*(t/input.tau)>2.1));
...
V_sqig_vec=I_1.*input.dL_nozz_hand(t);

[V_Cap_maxVal,V_Cap_maxInd]=max(V_Cap);
E_cap=0.5*input.C*(V_Cap_maxVal^2-V_Cap(1)^2);

% Calulcate current changes
L0_vec=input.L1+input.l_1+L_t_vec;
M_circ=input.k*sqrt(input.L1*input.L2);
dI2_vec=(V_Cap-I_2*input.R2-(M_circ*(V_sqig_vec-I_1*input.R1)./L0_vec))...
    ./(input.l_2+input.L2-(M_circ^2./L0_vec));
dI1_vec=(V_sqig_vec-I_1*input.R1-M_circ*dI2_vec)./(L0_vec);
% Calculate energy of each component
E_Cap=0.5*input.C*V_Cap(1:V_Cap_maxInd).^2;

E_in=zeros(V_Cap_maxInd,1);
for i=1:length(E_in)
    if i == 1
        E_in(i)=0;
    else
        E_in(i)=trapz(t(1:i),I_1(1:i).*V_sqig_vec(1:i));
    end
end
...
for i=1:length(E_gen)
    if i == 1
        ...
        E_gen(i)=trapz(t(1:i),I_1(1:i).*L_t_vec(1:i).*dI1_vec(1:i));
    end
end
...
figure(6)
plot(t*1e6,input.v_exp_hand(t)*1e-3)
