% Nathan Schilling
% 02/19/19
function E_gain = circuitModelFun(circuitInputParams)
    ...
    if circuitInputParams.graphDisplay
        ...
        grid on
        xlabel('Time since ignition ($\mu$sec)','interpreter','latex','fontsize',16)
        ylabel('Current (A)','interpreter','latex','fontsize',16)
        legend('Current on generator side','Current on capacitor side','location','southeast')
        
        figure(2)
        plot(t*10^6,I_2*1e-6)
        grid on
        xlabel('Time since ignition ($\mu$sec)','interpreter','latex','fontsize',16)
        ylabel('Current on load side (MA)','interpreter','latex','fontsize',16)
        
        figure(3)
        plot(t*10^6,y(:,2)*1e-3)
        grid on
        xlabel('Time since ignition ($\mu$sec)','interpreter','latex','fontsize',16)
        ylabel('Voltage across capacitor \textup{U}(kV)','interpreter','latex','fontsize',16)
    end
